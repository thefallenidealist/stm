...
int8_t i2c_start(uint8_t i2c_number)
{
	...
	switch (i2c_number)
	{
		case 1:
			...
			break;
		case 2:
			while (I2C_GetFlagStatus(I2C2, I2C_FLAG_BUSY));	// ne smije bit za ponovni i2c_start za EEPROM
			// timeout? TODO
...
/*
// stari write, rucni, neautomatizirani
...
int8_t i2c_write(uint8_t i2c_number, uint8_t slave_addr, uint8_t data)
{
	...
	// TODO ovdje start i stop za I2C provjeravaju drugacije flagove nego gornji _start _stop
...
int8_t i2c_read(uint8_t i2c_number, uint8_t slave_addr)
{
	...
	switch (i2c_number)
	{
		...
		case 2:
			i2c_start(2);
			i2c_sendAddr_rx(2, slave_addr);	// TODO, ako ce se samo interno koristit onda direktno stavit funkciju koju ovo pozove
			i2c_nack(2);
